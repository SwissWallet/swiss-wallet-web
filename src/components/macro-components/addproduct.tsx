...
export function AddNewProduct() {
    ...
    <div className="flex items-center w-full">
        <p className="text-red-700 text-center w-full font-medium text-lg sm:text-xl">{textAlert}</p>
    </div>

    <main className="flex flex-col md:flex-row items-center justify-around mt-10 md:mt-20 space-y-6 md:space-y-0">
        {!file ? (
            <InputImage dropzone={dropzone} />
        ) : (
            <HasImage file={file} removeFile={removeFile} />
        )}

        <form className="flex flex-col gap-4 w-full max-w-md md:w-2/4 justify-between p-4 md:p-8 lg:p-12">
            <UserInput type="text" placeholder="Ex: Camiseta Branca" value={name} onChange={(e) => setName(e.target.value)}>
                Título
            </UserInput>
            <UserInput type="text" placeholder="Camiseta Branca Básica" value={description} onChange={(e) => setDescription(e.target.value)}>
                Descrição
            </UserInput>
            <UserInput type="number" placeholder="Ex: 40,00" value={value} onChange={(e) => setValue(e.target.value)}>
                Valor
            </UserInput>
            <UserInput type="number" value={amount} onChange={(e) => setAmount(Number(e.target.value))}>
                Quantidade
            </UserInput>
            <UserSelect 
...
const InputImage = ({ dropzone }: InputImageProps) => {
    return (
        <div
            ...
            className='hover:cursor-pointer flex flex-col gap-20 hover:bg-zinc-100 bg-white shadow-xl p-16 w-full md:w-2/4 h-[440px] items-center text-center rounded-xl'>
...
const HasImage = ({ file, removeFile }: HasImageProps) => {
    ...
    return (
        <div className='text-zinc-500  flex flex-col hover:bg-zinc-100 bg-white shadow-xl p-5 gap-1 w-full md:w-2/4 h-[440px] rounded-xl'>
